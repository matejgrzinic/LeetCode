...
type testFunc func(*TreeNode, *TreeNode) bool
...
func Test(fn testFunc) {
	...
	equals(BinaryTree([]string{"1", "2", "3"}), BinaryTree([]string{"1", "2", "3"}), true)
	equals(BinaryTree([]string{"1", "2", "1"}), BinaryTree([]string{"1", "1", "2"}), false)
	equals(BinaryTree([]string{"1", "2"}), BinaryTree([]string{"1", "nil", "2"}), false)
...
func equals(p *TreeNode, q *TreeNode, expected bool) {
	numTests++
	got := testFunction(p, q)
	if got != expected {
		fmt.Println(getElements(p), getElements(q), "got:", got, "expected:", expected)
		failed++
	}
}
